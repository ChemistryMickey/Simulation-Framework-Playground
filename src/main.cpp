...
int main() {
  ...
  // Sensor/Actuators (hm... maybe this is where the fine point lies. Something
  // is both a sensor and an actuator)
  ...
  // Connect models that can't otherwise be connected at construction time due
  ...
  std::vector<Job> jobs{
      {.func =
           [&integrator]() { std::cout << integrator.dataTable << std::endl; },
       .frequency = 17,
       .phase = SimPhase::Integration},
      {.func = [&integrator]() { integrator.integrate(1.0 / 10.0); },
       .frequency = 10,
       .phase = SimPhase::Integration}};
